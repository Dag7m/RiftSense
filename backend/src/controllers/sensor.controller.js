...
async function ingestData(req, res) {
  try {
    ...
    if (recentData.length >= 100) {
      ...
      if (staLtaResult.triggered) {
        ...
        try {
          ...
          prediction = await mlClient.predict({ 
            features, 
            sensorData: chronologicalData 
          });
    ...
    socketUtil.emitSensorData(node.node_id, {
...
async function ingestBatchData(req, res) {
  try {
    ...
    // Emit batch update for real-time visualization
    if (recentData.length > 0) {
      socketUtil.emitSensorData(node.node_id, {
        node_id: node.node_id,
        batch: recentData.slice(0, batchData.length), // Send the newly ingested part
        detection: detectionResult
      });
    }
...
async function handleEarthquakeDetection(node, sensorData, prediction, staLtaResult) {
  ...
  try {
    ...
    if (pendingEvent) {
      ...
      const updatedEvent = await EventModel.update(pendingEvent.id, { confidence: newConfidence });

      // Emit update
      socketUtil.emitSeismicEvent({ ...updatedEvent, type: 'update', node_id: node.node_id });
      ...
      // Emit new event alert
      socketUtil.emitSeismicEvent({ ...newEvent, type: 'new', node_id: node.node_id });
